...
Example Run:
On starting (running) the solver2_file, after simply inputting a integer value for which function the solver will use (can choose from functions 1, 2, 3, or 4, unforunately 5 is still being worked on), inputting an integer value for number of iterations, and inputting some value for x_0 (can be positive or negative, can be a decmical or a whole number), the solver will put the value for x_0 through the chosen function's main and prime (derivative) form, before the use them all as variables for the formula for newton's method (x_n+1 = x_n + f(x)/f'(x)). It will run recursively for however many iterations the user has set to, and if that value is large enough eventually the user will notice that the final values for x_n and f(x) have settled on some consistently repeating value, which should be the convergence (a root) of the function. Depending on the direction it was approached (whether the input value for x_0 was positive or negative) the root may also be positive or negative, and may be only one of several roots for the function.
